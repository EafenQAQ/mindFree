    ...
    <div class="chat-container flex items-center justify-center h-screen gap-4 mt-10">
      ...
      <aside :class="[
        ...
        <div class="flex justify-between m-5" :class="{ plusBtn: !isSidebarOpen }">
          ...
          <button @click="toggleSidebar" class="btn btn-ghost hover:bg-white/30 rounded-full text-xl">
            ...
          </button>
        </div>
        ...
        <div ref="chatCards" class="chat-cards flex flex-col gap-1 max-h-[700px] p-4">
          <div v-for="chat in chatHistory" :key="chat.id" @click="switchChat(chat.id)"
            ...
            <div class="hamburger" :class="{ 'hamburger-hidden': !isSidebarOpen }">
              ...
              <ul class="dropdown menu w-fit rounded-box shadow-sm dropdown-right bg-white/80" popover :id="chat.id"
                :style="{ positionAnchor: chat.id }">
                <li class="text-black" @click.stop="editChatTitle(chat.id)">
                  <a>
                    <svg t="1742978391939" class="icon" viewBox="0 0 1024 1024" version="1.1"
                      xmlns="http://www.w3.org/2000/svg" p-id="1478" width="20" height="20">
                      <path
                ...
                <li class="text-red-500" @click.stop="deleteChat(chat.id)">
                  <a>
                    <svg t="1742978506942" class="icon" viewBox="0 0 1024 1024" version="1.1"
                      xmlns="http://www.w3.org/2000/svg" p-id="2991" width="20" height="20">
                      <path
      ...
      <main
        ...
        <ChatBar class="self-start" :chatHistory="chatHistory" @addNewChat="
          (chatID) => {
            startNewChat(chatID);
          }
        " @editChatTitle="editChatTitle" @deleteChat="deleteChat" @propSwitchChat="
          (chatID) => {
            switchChat(chatID);
          }
        " />
